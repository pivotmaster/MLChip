...
vector<float> softmax(const vector<float> &logits) {
    ...
    transform(logits.begin(), logits.end(), back_inserter(exp_values),
              [](float logit) { return exp(logit); });

    float sum = accumulate(exp_values.begin(), exp_values.end(), 0.0f);
    vector<float> probabilities;
    transform(exp_values.begin(), exp_values.end(),
              back_inserter(probabilities),
              [sum](float exp_value) { return exp_value / sum; });
...
void printResult(FCTensor result, vector<string> label,
    ...
    for (int i = 0; i < result.size(); i++) {
        ...
        for (int j = 0; j < possibility.size(); j++) {
            indexedPossibilityWithScore.push_back(
                make_tuple(j, possibility[j], result[i][j]));
        }
        vector<tuple<int, float, float>> topTen(10);
        partial_sort_copy(indexedPossibilityWithScore.begin(),
                          indexedPossibilityWithScore.end(), topTen.begin(),
                          topTen.end(),
                          [](const tuple<int, float, float> &left,
                             const tuple<int, float, float> &right) {
                              return get<1>(left) > get<1>(right);
                          });
        for (auto &[index, val, originalScore] : topTen) {
            cout << " " << setw(3) << index << " |  " << setw(5) << fixed
                 << setprecision(4) << originalScore << " |    " << setw(10)
                 << setprecision(5) << val * 100 << " | " << label[index]
                 << endl;
        }
    }
}
