...
Tensor AlexNet::ApplyPadding(Tensor &input, int padding) {
    ...
    Tensor output(batch_size,
                  Pic(channels, vector<vector<float>>(
                                    height + 2 * padding,
                                    vector<float>(width + 2 * padding, 0.0))));
...
Tensor AlexNet::Conv(WeightConv &weight_conv, WeightConvBias &weight_bias,
    ...
    Tensor output(
        batch_size,
        Pic(ch_out, vector<vector<float>>(output_height,
                                          vector<float>(output_width, 0.0))));
                    ...
                    for (int ci = 0; ci < ch_in; ++ci) {
                        for (int kh = 0; kh < kernel_y; ++kh) {
                            for (int kw = 0; kw < kernel_x; ++kw) {
                                if (h * stride + kh < input_height &&
                                    w * stride + kw < input_width) {
                                    sum += padded_input[b][ci][h * stride + kh]
                                                       [w * stride + kw] *
                                           weight_conv[co][ci][kh][kw];
                                    mult_add_count += 1;
                                }
                            }
                        }
...
Tensor AlexNet::MaxPooling(const Tensor &input, int pool_size_x,
                           int pool_size_y, int stride) {
    int batch_size = input.size();
    ...
    Tensor output(
        ...
        vector<vector<vector<float>>>(
            depth, vector<vector<float>>(output_height,
                                         vector<float>(output_width, 0.0))));
                ...
                for (int w = 0; w < output_width; ++w) {
                    ...
                    for (int i = 0; i < pool_size_y; ++i) {
                        for (int j = 0; j < pool_size_x; ++j) {
                            ...
                            if (h_index < input_height &&
                                w_index < input_width) {
                                max_val =
                                    max(max_val, input[b][d][h_index][w_index]);
                            }
                        }
                    }
...
Tensor AlexNet::AdaptiveAvgPool(const Tensor &input, int output_height,
                                int output_width) {
    int batch_size = input.size();
    int channels = input[0].size();
    int input_height = input[0][0].size();
    ...
    Tensor output(
        batch_size,
        vector<vector<vector<float>>>(
            channels, vector<vector<float>>(output_height,
                                            vector<float>(output_width, 0.0))));
...
FCTensor AlexNet::Flatten(const Tensor &input) {
    ...
    int flat_size =
        input[0].size() * input[0][0].size() * input[0][0][0].size();
...
FCTensor AlexNet::FullyConnected(const FCTensor &input, const WeightFC &weights,
